...
Here is an example of four records that represent the same individual using data that is in the `representr` package. 
...
Before we perform prototyping to get a representative data set using `representr`, we must first perform record linkage. For the examples in this vignette, we will use `blink` [@blink], which is a Bayesian record linkage model [@steorts2015entity]. For this example, we will run only a small number of iterations, which will certainly not have converged. For a real example, we would want to let this model run longer.
...
random_id <- represent(rl_reg1, lambda, "proto_random")
